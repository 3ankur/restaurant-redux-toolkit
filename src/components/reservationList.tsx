...
import { addReservation, removeReservation, ReservationEntity } from "../features/reservationSlice";
...
function ReservationList() {
    ...
    const addDining = (reservation:ReservationEntity) =>{
        ...
        dispatch(addCustomerDining({
            ...reservation,
            food:[]
        }))
    }

    return (<div className="reservation-list-container">

        {
            reservations.map((item) => {
                return (<div key={item?.id} onClick={()=>addDining(item)} className="reservation-name">{item.name}</div>)
            })
        }
        <div className="add-reservation">
            <input type="text" ref={nameText} />
            <button onClick={addNewReservation}>Add</button>
        </div>
    </div>)
}
